...
# create random data with two classes; two input X and one binary output y
...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=2)
    # returns 4 sets(2 training, 2 test), with respective values from original lists
...
def main(X_train, X_test, y_train, y_test):
    ...
    for i, test_item in enumerate(X_test):
        ...
        nearest = np.argmin(distances) # returns index of lowest elem in distances

        # create a line connecting the points for the chart
        lines.append(np.stack((test_item, X_train[nearest])))

        # add your code here:
        y_predict[i] = y_train[nearest] #fills y_predict list with the "nearest" data in y_train
